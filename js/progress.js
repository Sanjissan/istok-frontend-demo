...
(function () {
  ...
  let LIVE_ROWS = []; // Full DB rows
  const PROGRESS = {}; // Map: "RackName|ProcessName" -> StatusCode (1-7)
  ...
  // ==========================================================
  // 3. RACK DATA (SU_RACKS & CELL_RACKS)
  // ==========================================================
  ...
  function statusLabelToDot(label){
    ...
    return ({
      yellow:"🟡", orange:"🟠", cyan:"🔵", blue:"🔷", green:"🟢", red:"🔴", purple:"🟣", default:"⚪",
    })[k] || "⚪";
  }
  ...
  // ==========================================================
  // 5. UI LOGIC (Map, Selectors, Clicks, Zoom)
  // ==========================================================
  ...
  function buildGpuRacksFromSuEls(suEls){
    ...
    for (const [suKey, set] of Object.entries(bySu)){
      ...
      out[suKey] = Array.from(set).filter(code => !masterName || norm(code) !== masterName).map(code => ({
          id: `${code}-SU${suKey}`, name: code, type: "GPU", processes: (PROCESS_BY_TYPE["GPU"] || [])
  ...
  function ensureRackSelectInSelected(panel){
    ...
    wrap.innerHTML = `<button class="btn-ghost" id="ptClearSel" type="button">Clear</button><div style="height:10px"></div><div class="label" style="margin-bottom:6px">Rack (within selected SU)</div><select id="ptRack" class="pt-select" disabled></select><div id="ptRackHint" class="pt-hint"></div>`;
    g.appendChild(wrap);
  ...
  function bindRackProcessSelect(){
    return { sel: document.getElementById("ptRackProcess"), hint: document.getElementById("ptRackProcessHint") };
  }
  ...
  document.addEventListener("DOMContentLoaded", async function(){
    ...
    let baseMatrixW = null, baseMatrixH = null, mapZoom = 1;

    function setMapZoom(z){
      if (!matrixRoot || !matrixZoom) return;
      if (baseMatrixW === null) { baseMatrixW = matrixRoot.offsetWidth; baseMatrixH = matrixRoot.offsetHeight; }
      if (z < 0.6) z = 0.6; if (z > 1.8) z = 1.8;
      mapZoom = z;
      matrixRoot.style.transformOrigin = "0 0";
      matrixRoot.style.transform = "scale(" + mapZoom + ")";
      matrixZoom.style.width = Math.round(baseMatrixW * mapZoom) + "px";
      matrixZoom.style.height = Math.round(baseMatrixH * mapZoom) + "px";
      if (zoomPct) zoomPct.textContent = Math.round(mapZoom * 100) + "%";
    }
    setMapZoom(1);
    if(zoomInBtn) zoomInBtn.onclick = () => setMapZoom(mapZoom + 0.1);
    if(zoomOutBtn) zoomOutBtn.onclick = () => setMapZoom(mapZoom - 0.1);
    if(zoomResetBtn) zoomResetBtn.onclick = () => setMapZoom(1);
    ...
    function renderSelected() {
      ...
      // Attempt to auto-select relevant process
      const currentMainProc = resolveProcessKey(processSelect.value);
      if(procs.includes(currentMainProc)) rackProcessSelect.value = currentMainProc;
      ...
      if (activeProc && activeProc !== "ALL") {
        ...
        if (tpl) Object.keys(tpl).forEach(k => statusSelect.appendChild(opt(k, `${statusLabelToDot(tpl[k])} ${tpl[k]}`)));
        
        const code = getCode(selected.suKey, selected.rackId, activeProc);
        statusSelect.value = String(code);
        statusSelect.disabled = false;
        
        // Colorize select box
        const label = tpl ? tpl[code] : "";
        if(label) statusSelect.dataset.statuskey = statusLabelToKey(label);
    ...
    processSelect.addEventListener("change", () => {
      // Clear selection on process change to avoid confusion
      selected = null;
      suEls.forEach(x => x.classList.remove("pt-selected"));
      renderSelected();
      applyFilters(); 
    ...
    statusSelect.addEventListener("change", () => {
        const tpl = PROCESS_TEMPLATES[rackProcessSelect.value];
        const code = statusSelect.value;
        if(tpl && tpl[code]) statusSelect.dataset.statuskey = statusLabelToKey(tpl[code]);
    ...
    // --- SAVE LOGIC (The Critical Fix) ---
    if (applyBtn) {
      applyBtn.addEventListener("click", async () => {
        try {
          const who = await pickResponsible();
          if (!who) { if (applyHint) applyHint.textContent = "Responsible is required."; return; }
          ...
          // Clean status text (remove the colored dot)
          const statusTextRaw = statusSelect.options[statusSelect.selectedIndex]?.textContent || "";
          const statusName = statusTextRaw.replace(/^[^\w\d]+/, "").trim(); // Remove leading dots/emojis
          
          const note = (noteEl?.value || "").trim();

          if (!rackName || !processName || !statusName || processName === "ALL") {
            if (applyHint) applyHint.textContent = "Select rack, specific process and status first."; return;
          }
          ...
          if (!row || !row.rack_process_run_id) {
            if (applyHint) applyHint.textContent = `Error: Row not found in DB for ${rackName} / ${processName}`; return;
          }
          ...
          if (!statusId) { if (applyHint) applyHint.textContent = "Error: Could not determine Status ID."; return; }
          ...
          if (applyHint) applyHint.textContent = "Saved ✅ Refreshing...";
          setTimeout(() => window.location.reload(), 500);

        } catch (e) {
          console.error(e);
          if (applyHint) applyHint.textContent = `Error: ${e.message || e}`;
        } finally {
