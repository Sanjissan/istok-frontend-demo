...
(function () {
  ...
  // ==========================================================
  // 2. LIVE BACKEND CACHE (used by Apply)
  // ==========================================================
  let LIVE_ROWS = [];
...
const CELL_RACKS = { 
  ...
  const PROGRESS = {
    "LRF-SU90|ROCE T1: AS-T1/R.T1-T2": 4,
    "GPU-SU90|GPU AEC": 2,
    "GPU-SU90|GPU CAT6": 1,
    "LRA-SU89|ROCE T1: AS-T1/R.T1-T2": 6,
    "GPU-SU89|GPU AEC": 7,
    "LMK-SU65|ROCE T1: AS-T1/R.T1-T2": 7,
    "GPU-SU65|GPU AEC": 7,
  };
...
function isQCDoneForProc(suKey, rack, proc){
  ...
  return c >= completion;
    ...
    if (applyBtn) {
      async function getSelectedText(selectEl) {
      const optEl = selectEl?.options?.[selectEl.selectedIndex];
      return (optEl?.textContent || optEl?.value || "").trim();
    }

    function rackBaseFromRackSelectText(text) {
      // "LAC • ROCE T1" -> "LAC"
      const s = String(text || "").trim();
      return s.includes("•") ? s.split("•")[0].trim() : s;
    }

    function resolveActiveProcess(processSelect, rackProcessSelect) {
      // Prefer rack-specific process dropdown when enabled
      if (rackProcessSelect && !rackProcessSelect.disabled && rackProcessSelect.value) return rackProcessSelect.value;
      return processSelect?.value || "ALL";
    }
    ...
    applyBtn.addEventListener("click", async () => {
      try {
        ...
        if (!who) {
          if (applyHint) applyHint.textContent = "Responsible is required.";
          return;
        }

        // 2) Read selection from UI
        const rackNameBase = rackBaseFromRackSelectText(await getSelectedText(rackSelect)); // e.g. "LAC"
        const processName = resolveActiveProcess(processSelect, rackProcessSelect);         // e.g. "ROCE T1: ..."
        const statusLabel = resolveStatusLabel(statusSelect);                               // e.g. "PATCHING DONE"
        const note = (noteEl?.value || "").trim();

        if (!rackNameBase || !processName || processName === "ALL" || !statusLabel) {
          if (applyHint) applyHint.textContent = "Select rack, specific process and status first.";
          return;
        }

        // 3) Ensure LIVE_ROWS is fresh (so we can map rack/process -> rack_process_run_id/status_id)
        if (!Array.isArray(LIVE_ROWS) || LIVE_ROWS.length === 0) {
          await syncProgressFromBackend();
        }

        const row = (LIVE_ROWS || []).find(r =>
          String(normalizeRackBase(r)).toLowerCase() === String(rackNameBase).toLowerCase() &&
          String(normalizeProcessName(r)).toLowerCase() === String(processName).toLowerCase()
        );

        if (!row || !row.rack_process_run_id) {
          if (applyHint) applyHint.textContent = `Can't map selection to DB row. rack="${rackNameBase}", process="${processName}"`;
          return;
        }

        // 4) Try to determine status_id (preferred), otherwise fallback to status_name.
        // Many backends accept either status_id or status_name — we send both when possible.
        let status_id = null;

        // If the selected option is "1..7" — treat it as code and use template label
        const rawVal = String(statusSelect?.value || "").trim();
        const maybeCode = Number(rawVal);
        if (Number.isFinite(maybeCode) && maybeCode >= 1 && maybeCode <= 7) {
          // We still prefer backend id, but we can at least normalize label
          // (statusLabel already contains the template label)
        }

        const wantedUpper = String(statusLabel).trim().toUpperCase();
        const sameLabel = (LIVE_ROWS || []).find(r => String(r.current_status || r.status || r.status_name || "").trim().toUpperCase() === wantedUpper);
        if (sameLabel && sameLabel.status_id != null) {
          const n = Number(sameLabel.status_id);
          if (Number.isFinite(n)) status_id = n;
        }
        ...
        await apiUpdateRunStatus(row.rack_process_run_id, {
          status_id: status_id,
        ...
        // 6) Optimistic UI update (no hard reload)
        const tpl = PROCESS_TEMPLATES[processName];
        const code = tpl ? statusToCode(tpl, statusLabel) : 1;
        PROGRESS[`${rackNameBase}|${processName}`] = code;

        if (applyHint) applyHint.textContent = "Saved ✅";
        applyFilters();
        renderSelected();

        // Also refresh from server in background (best-effort)
        try {
          await syncProgressFromBackend();
          applyFilters();
          renderSelected();
    ...
    /* Initial paint (local/default) */
    applyFilters();
    updateInteractivity();
    renderSelected();

    // Initial sync from backend (cross-device) then repaint
    (async () => {
      await syncProgressFromBackend();
      setView(currentView);
      renderAll();
