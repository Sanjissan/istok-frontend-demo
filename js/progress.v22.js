...
const CELL_RACKS = { 
  ...
  function ptRackIdFromRow(row, suKey){
    ...
    // If backend sends a short rack "name" (e.g., "LSL") without SU suffix, prefer the exact UI rack id from DOM.
    // This fixes cases where the static rack list / regex does not include that rack code (common for higher SU numbers).
    const suNum = suNumFromKey(suKey);
    if (suNum && !/\-SU\d+$/i.test(rackId)) {
      const wanted = `${rackId}-SU${suNum}`;
      try {
        const opts = document.querySelectorAll("option");
        for (const o of opts) {
          const v = (o.value || "").trim();
          if (v && norm(v) === norm(wanted)) return v;
        }
      } catch (e) {}
    }

    // Already a UI rack id
    if (/\-SU\d+$/i.test(rackId)) return rackId;

    // Known backend rack names that should be SU-scoped in UI
    if (/^(LAC|LAH|LAM|LAR|LAW|LBB|LBG|LBL|LBQ|LBV|LCA|LCF|LCK|LCP|LCU|LCZ|LDE|LDJ|LDO|LDT|LDY|LED|LEI|LEN|LES|LEX|LFC|LFH|LFM|LFR|LFW|LGB|LGG|LGL|LGQ|LGV|LHA|LHF|LHK|LHP|LHU|LHZ|LIE|LIJ|LIO|LIT|LIY|LJD|LJI|LJN|LJS|LJX|LKC|LKH|LKM|LKR|LKW|LLB|LLG|LLL|LLQ|LLV|LMA|LMF|LMK|LMP|LMU|LMZ|LNE|LNJ|LNO|LNT|LNY|LOD|LOI|LON|LOS|LOX|LPC|LPH|LPM|LPR|LPW|LQB|LQG|LQL|LQQ|LQV|LRA|LRF|LRK|LRP|LRU|LRZ|LSE|LSJ|GPU)$/i.test(rackId) && suNum) {
      rackId = rackId + "-SU" + suNum;
      return rackId;
    }

    // Generic fallback for short rack codes (3-4 letters/digits) that backend stores as `name`
    // but UI expects `<name>-SU<suNum>`.
    if (suNum && /^[A-Z0-9]{3,4}$/i.test(rackId)) {
      return String(rackId).toUpperCase() + "-SU" + suNum;
    }
...
function ptApplyBackendRowToUI(row) {
  ...
  if (!suKey || !procKey) return;

  const baseRackId = ptRackIdFromRow(row, suKey);
  const rackIds = ptCandidateRackIdsForRow(suKey, row, baseRackId);
  ...
  if (statusName != null) {
    ...
    if (code != null) {
      for (const rid of rackIds) {
        setCodeFromBackend(suKey, rid, procKey, code);
      }
    }
  }
  ...
  if (note != null && String(note).trim() !== "") {
    for (const rid of rackIds) {
      setStoredNote(suKey, rid, procKey, String(note).trim());
    }
  }
}
