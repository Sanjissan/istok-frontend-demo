...
const CELL_RACKS = { 
  ...
  async function ptPersistToBackend(suKey, rackId, procKey, code, noteText){
    ...
    if (!runInfo) {
      throw new Error(`No rack_process_run_id for selected SU/rack/process: su=${suStr}, rack=${rackStr}, process=${procStr}`);
    }

    const runId = (typeof runInfo === "object" && runInfo) ? Number(runInfo.runId) : Number(runInfo);
    const processId = (typeof runInfo === "object" && runInfo) ? Number(runInfo.processId)
