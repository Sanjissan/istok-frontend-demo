...
const CELL_RACKS = { 
  ...
  async function ptBootstrapFromBackend(){
    ...
    try {
      ...
      for (const row of (rows || [])){
        ...
        PT_DB.runIndex.set(`${rackId}|${procKey}`, { runId, processId: procId });
        PT_DB.runIndex.set(`${suKey}|${rackId}|${procKey}`, { runId, processId: procId });
        PT_DB.runIndex.set(`${norm(rackId)}|${norm(procKey)}`, { runId, processId: procId });
        PT_DB.runIndex.set(`${norm(suKey)}|${norm(rackId)}|${norm(procKey)}`, { runId, processId: procId });
  ...
  async function ptPersistToBackend(suKey, rackId, procKey, code, noteText){
    ...
    let runInfo = PT_DB.runIndex.get(`${rackId}|${procKey}`)
      || PT_DB.runIndex.get(`${suKey}|${rackId}|${procKey}`)
      || PT_DB.runIndex.get(`${norm(rackId)}|${norm(procKey)}`)
      || PT_DB.runIndex.get(`${norm(suKey)}|${norm(rackId)}|${norm(procKey)}`);

    if (!runInfo) {
      const suNumFromSuKey = String(suKey || "").replace(/^SU\s*/i, "").trim();
      const suNumFromRackId = (String(rackId || "").match(/SU\s*([0-9]+)/i) || [])[1] || "";
      const suNum = suNumFromRackId || suNumFromSuKey;
      if (suNum) {
        const altRackId = `GPU-SU${suNum}`;
        runInfo = PT_DB.runIndex.get(`${altRackId}|${procKey}`)
          || PT_DB.runIndex.get(`${norm(altRackId)}|${norm(procKey)}`)
          || PT_DB.runIndex.get(`${suNum}|${altRackId}|${procKey}`)
          || PT_DB.runIndex.get(`${norm(suNum)}|${norm(altRackId)}|${norm(procKey)}`);
      }
    }

    if (!runInfo) throw new Error(`No rack_process_run_id for selected rack/process: ${rackId}|${procKey}`);
    ...
    if (!runId) throw new Error(`No rack_process_run_id for selected rack/process: ${rackId}|${procKey}`);
