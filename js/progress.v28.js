...
console.log("[BOOT] after apply runIndex.size=", window.PT_DB && window.PT_DB.runIndex && window.PT_DB.runIndex.size);
 ...
 function loadNoteForSelection(){
  try {
    ...
    // 1) сначала берём из локального кэша (если есть)
    const getter =
    ...
    const isCellKey = /^LU\d+_ROW\d+_/i.test(suRaw);

    const suNum = (!isCellKey)
      ? String(suNumFromKey(suRaw) || suRaw).replace(/^SU\s*/i, "").trim()
      : "";

    const suUI  = (suNum && typeof suKeyToUI === "function") ? suKeyToUI(suNum) : (suNum ? ("SU"+suNum) : "");
    ...
    const racks = Array.from(new Set([
      rackRaw,
      rackBase,
      (rackBase && suNum ? `${rackBase}-SU${suNum}` : null),
    ].filter(Boolean)));
    ...
    // 2) если пусто — берём "истину" с backend
    //    НО: теперь lookup строим правильно: process_id + (su_key ИЛИ lu+row)
    if (!v && window.PT_REST && typeof window.PT_REST.fetchJSON === "function") {
      const pid = (window.PT_DB && window.PT_DB.procDescToId)
        ? (window.PT_DB.procDescToId.get(norm(targetProc)) || 0)
        : 0;

      // если нет process_id — не дёргаем бек
      // если нет process_id — не дёргаем бек
if (!pid) return;

// строим lookup строго как ожидает backend
const params = new URLSearchParams();
params.set("rack_name", String(rackBase || "").trim());
params.set("process_id", String(pid));

if (!isCellKey && suNum) {
  params.set("su_key", String(suNum));
} else {
  // cellKey: LU7_ROW12_...
  const m = String(suRaw || "").match(/^LU(\d+)_ROW(\d+)_/i);
  if (m) {
    params.set("lu", m[1]);       // "7"
...
const url = "/api/runs/lookup?" + params.toString();
  ...
  const backendNote = row.note != null ? String(row.note).trim() : "";
          ...
          // обновим textarea, если пользователь прямо сейчас не печатает
          if (document.activeElement !== noteEl || !String(noteEl.value || "").trim()) {
            noteEl.value = backendNote;
          }
        })
        .catch(()=>{}) // 404 = ок (строки может не быть)
        .finally(() => {
