/* PT_FIX_V17_DEBUG_TOOLS: bootstrap from /api/runs?limit=5000 so UI reflects DB after reload */
...
function ptApplyBackendRowToUI(row) {
  ...
  if (statusName != null) {
    ...
    if (code != null) {
      for (const rid of rackIds) {
        setCode(suKey, rid, procKey, code);
      }
    }
  }
...
function isQCDoneForProc(suKey, rack, proc){
  ...
  return c >= completion;
    ...
    ptBootstrapFromBackend().then(function(){
      // PT_FIX_V17_DEBUG: expose helpers to inspect whether backend state is applied
      window.PT_DEBUG = window.PT_DEBUG || {};
      window.PT_DEBUG.getCode = function(su, rack, proc){
        try { return getCode(String(su||""), String(rack||""), String(proc||"")); }
        catch(e){ return { error: String(e) }; }
      };
      window.PT_DEBUG.progressInfo = function(){
        try { const keys = Object.keys(PROGRESS); return { count: keys.length, sample: keys.slice(0, 50) }; }
        catch(e){ return { error: String(e) }; }
      };
      window.PT_DEBUG.run575 = function(){
        const get = (window.PT_API && typeof window.PT_API.getRuns === "function")
          ? window.PT_API.getRuns(5000)
          : window.PT_REST.fetchJSON("/api/runs?limit=5000&_=" + Date.now());
        return Promise.resolve(get).then(rows => (rows||[]).find(r => Number(r.rack_process_run_id) === 575));
      };
      applyFilters();
      updateInteractivity();
      renderSelected();
