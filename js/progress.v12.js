...
const CELL_RACKS = { 
  ...
  function ptRackIdFromRow(row, suKey){
    ...
    if (/^(LAC|LAH|LAM|LAR|LAW|LBB|LBG|LBL|LBQ|LBV|LCA|LCF|LCK|LCP|LCU|LCZ|LDE|LDJ|LDO|LDT|LDY|LED|LEI|LEN|LES|LEX|LFC|LFH|LFM|LFR|LFW|LGB|LGG|LGL|LGQ|LGV|LHA|LHF|LHK|LHP|LHU|LHZ|LIE|LIJ|LIO|LIT|LIY|LJD|LJI|LJN|LJS|LJX|LKC|LKH|LKM|LKR|LKW|LLB|LLG|LLL|LLQ|LLV|LMA|LMF|LMK|LMP|LMU|LMZ|LNE|LNJ|LNO|LNT|LNY|LOD|LOI|LON|LOS|LOX|LPC|LPH|LPM|LPR|LPW|LQB|LQG|LQL|LQQ|LQV|LRA|LRF|LRK|LRP|LRU|LRZ|LSE|LSJ|GPU)$/i.test(rackId) && (suNumFromKey(suKey) !== "")) {
      rackId = rackId + "-SU" + suNumFromKey(suKey);
    }
    return rackId;
  }
  ...
  async function ptBootstrapFromBackend(){
    ...
    try {
      ...
      for (const row of (rows || [])){
        ...
        // Some backends store runs by "rack_name" (e.g. GPU) while the UI uses
        // "position" rack ids (e.g. LSL-SU96) for GPU racks.
        // To keep UI in sync after reload, also map the run to *all* racks in this SU
        // whose type matches the backend rack_type (if available).
        const rackType = String(ptPick(row, ["rack_type","rackType","type"]) || "").trim();
        const candidateRackIds = new Set([rackId]);
        try {
          const suRacks = racksForSU(suKey) || [];
          if (rackType) {
            for (const r of suRacks) {
              if (!r) continue;
              if (norm(r.type) === norm(rackType)) candidateRackIds.add(String(r.id));
            }
          }
        } catch {}
        ...
        for (const rid of candidateRackIds) {
          const rackStr2 = String(rid).trim();
          if (!rackStr2) continue;

          PT_DB.runIndex.set(`${rackStr2}|${procKey}`, { runId, processId: procId });
          PT_DB.runIndex.set(`${suKey}|${rackStr2}|${procKey}`, { runId, processId: procId });
          PT_DB.runIndex.set(`${norm(rackStr2)}|${norm(procKey)}`, { runId, processId: procId });
          PT_DB.runIndex.set(`${norm(suKey)}|${norm(rackStr2)}|${norm(procKey)}`, { runId, processId: procId });
          ...
          if (statusName != null) {
            ...
            if (code != null) setCode(suKey, rackStr2, procKey, code);
          }

          const note = ptPick(row, ["note","notes","comment"]);
          if (note != null && String(note).trim() !== "") {
            setStoredNote(suKey, rackStr2, procKey, String(note).trim());
          }
        }
      }
      PT_DB.loaded = true;
...
const runId = (typeof runInfo === "object" && runInfo) ? Number(runInfo.runId) : Number(runInfo);const processId = (typeof runInfo === "object" && runInfo) ? Number(runInfo.processId)
    ...
    return window.PT_REST.apiUpdateRunStatus(runId, {
      status_id,
