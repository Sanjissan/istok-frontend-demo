...
// ==========================================================
// PT_FIX_DB_SOURCE_OF_TRUTH:
// After any successful write, re-fetch latest runs from backend and re-apply to UI.
// This ensures UI always reflects DB even if the row wasn't in the initial cache.
// ==========================================================
let __pt_sync_inflight = null;
async function ptSyncRunsFromBackend() {
  if (__pt_sync_inflight) return __pt_sync_inflight;
  __pt_sync_inflight = (async () => {
    try {
      ...
      // ignore: UI will still have optimistic update via "updated" row
    } finally {
      __pt_sync_inflight = null;
    }
  })();
  return __pt_sync_inflight;
}
...
const runId = (typeof runInfo === "object" && runInfo) ? Number(runInfo.runId) : Number(runInfo);const processId = (typeof runInfo === "object" && runInfo) ? Number(runInfo.processId)
    ...
    // Re-sync from backend to ensure full DB truth in UI
    await ptSyncRunsFromBackend();
