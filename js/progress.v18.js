/* PT_FIX_V18_DOM_RACK_MAP: bootstrap from /api/runs and map backend rack_name/type to UI slot rack ids via DOM when needed */
...
function ptDomRackIdsForSUAndType(suNum, wantType, wantName){
  try {
    ...
    for (const o of opts){
      ...
      const nv = norm(v);
      const nt = norm(t);
      // If we know desired type/name (e.g., GPU), prefer matching options
      let ok = true;
